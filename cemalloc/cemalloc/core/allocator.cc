...
int ce_posix_memalign(void **memptr, size_t alignment, size_t sz) {
    ...
    if(result)
    {
        result=cxl_posix_memalign(memptr, alignment, sz);
    }
    hook_guard = false;
...
void *ce_memalign(size_t alignment, size_t sz) {
    ...
    if(addr== nullptr)
    {
        addr=cxl_memalign(alignment, sz);
    }
    hook_guard = false;
...
void *ce_aligned_alloc(size_t alignment, size_t sz) {
    ...
    if(addr== nullptr)
    {
        addr=cxl_aligned_alloc(alignment, sz);
    }
    hook_guard = false;
